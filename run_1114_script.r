...
# Source functions file and NONMEM .ctl reference txt
  master.dir <- "E:/hscpw-df1/Data1/Jim Hughes/2016/07_noBOV" ### Directory containing source files
  setwd(master.dir)
...
# Specify run characteristics
  ...
  rundf <- data.frame(
    RUN = unlist(llply(5:6, function(i) run.order + i*2)), #rep(number.of.runs, each=number.of.scenarios)
    SCEN = rep(c(1:9,1:6), times = 2), #opposite of above
    RUV.TYPE = rep(c(rep(1, 9),rep(2, 6)), 2),
    RUV.BLQ = rep(c(rep(c(0.2, 0.15, 0.1), 3),rep(c(0.1, 0.5), 3)), 2),
    F1.POP = rep(c(rep(f1.order, each = 3), rep(f1.order, each = 2)), 2), #changing Frel of generic
    F1.BSV = rep(c(rep(bsv.order, each = 3), rep(bsv.order, each = 2)), 2), #changing BSV on frel
    BLQ = rep(0.01, 30),  #Run 2 - raised LLOQ
    RUV.PROP = rep(0.05, 30),  #Run 3 - increased proportional RUV
    SS.TYPE = rep(1, 30),  #Run 4 - reduced sampling schedule
    KA.TYPE = rep(1, 30),  #Run 5 - 20% lower generic KA
    BOV.TYPE = c(rep(2, 15), rep(3, 15)))  #Run 6 & 7 - BOV testing
