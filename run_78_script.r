...
# Source functions file and NONMEM .ctl reference txt
  master.dir <- "E:/hscpw-df1/Data1/Jim Hughes/2016/04_SS" ### Directory containing source files
  setwd(master.dir)
...
# Specify run characteristics
  ...
  rundf <- data.frame(
    RUN = run.order + 6, #rep(number.of.runs, each=number.of.scenarios)
    ...
    SS.TYPE = rep(2, 15),  #Run 4 - reduced sampling schedule
    KA.TYPE = rep(1, 15),  #Run 5 - 20% lower generic KA
